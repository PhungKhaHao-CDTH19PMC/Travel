...
class Trung extends StatefulWidget {
  String username;
  Trung({Key? key, required this.username}) : super(key: key);

  @override
  _TrungState createState() => _TrungState();
}
int _iApi=0;
String _urlFind = "http://10.0.2.2:8000/doan/api/tim_kiem.php/?find=";
String _iUrl="";
class _TrungState extends State<Trung> {
  Iterable s = [];
  bool isUpdate = true;
...
Widget detail(int index){
  ...
  Widget build(BuildContext context) {
    API(url: "http://10.0.2.2:8000/doan/api/lay_dia_danh_trung.php")
          .getDataString()
          .then((value) {
          s = json.decode(value);
          isUpdate = false;
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Địa điểm ở miền trung'),
      ),
      body: ListView(
        children: [
        buildList()
